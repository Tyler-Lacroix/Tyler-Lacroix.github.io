...
{
public:
    int dice[5] = { 0, 0, 0, 0, 0 }; // player's list of dice
...
int countDice(int, int, player[]);
void callLiar(player[], int);
void displayMenu(player[], int, int, guesses);
void readRules();
...
void playerInit(player player[], int numplayers) // Initialize each player's dice set
{
    for (int i = 1; i <= numplayers; i++) // Loop to iterate through players
    {
        ...
        for (int b = 0; b < 6; b++) // Loop to assign dice values
        {
...
guessTracker makeGuess(guesses& guess) // Provide functionality to allow player to guess number of dice in play.
{
    ...
    while (!validGuess)
    {
        ...
        {
            cout << "You must guess more than that." << endl;
        }
    }

    cout << "What dice are you guessing?" << endl;
    cin >> diceChoice;
...
int countDice(int inputDiceValue, int numPlayers, player player[]) // Function to count dice faces
{
    int count = 0;
    ...
    for (int i = 0; i <= numPlayers; i++) // loop to iterate through all players.
    {
        ...
        for (int b = 0; b < 5; b++) // loop to iterate through a players dice.
        {
...
void callLiar(player players[], int numplayers) // Counts total dice in play, and determines the winner/loser
{
    int targetplayer;
    ...
    if (players[targetplayer].hasGuessed == true)
    {
        ...
        cout << "There are a total of " << (numplayers * 5) << " dice in play." << endl << "Among them there are:" << endl;
        cout << countDice(1, numplayers, players) << " 1s in play," << endl
             << countDice(2, numplayers, players) << " 2s in play," << endl
             << countDice(3, numplayers, players) << " 3s in play," << endl
             << countDice(4, numplayers, players) << " 4s in play," << endl
             << countDice(5, numplayers, players) << " 5s in play, and" << endl
             << countDice(6, numplayers, players) << " 6s in play." << endl << endl;
...
void displayMenu(player players[], int curPlayer, int numPlayer, guesses guess) // Main menu and primary game logic.
{
    int input;
    ...
    while (continueLoop) // loop until logic loop breaks
    {
        ...
        {
            case 1:
                printDice(5, players[curPlayer].dice);
                cout << endl;
...
int main()
{
    ...
    displayMenu(playersList, curPlayer, numplayers, guess); // Initiate main gameplay loop.
